...
double getIntegralS(std::function<double (double)>& fcn,
  ...
  while (status) {
    ...
    if (relerr < 1.e-3) {
      ...
      relerr *= 1.1;
      if(status) {
	std::cout << "AS: Warning: tolerance increased to "
		  << relerr << std::endl;
      }
    }
...
double getIntegral(std::function<double (double)>& fcn,
		   double a, double b, double& error) {
  int N = 1000000;
  gsl_error_handler_t * old_handler=gsl_set_error_handler_off();
  gsl_integration_workspace* w =
  ...
  while (status) {
    ...
    if (relerr < 1.e-3) {
      ...
      relerr *= 1.1;
      if(status) {
	std::cout << "A: Warning: tolerance increased to "
		  << relerr << std::endl;
      }
    }
