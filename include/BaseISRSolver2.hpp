#ifndef _BASE_ISRSOLVER2_HPP_
#define _BASE_ISRSOLVER2_HPP_
...
class BaseISRSolver2;
double* extractECMPointer(BaseISRSolver2*);
double* extractECMErrPointer(BaseISRSolver2*);
double* extractVCSPointer(BaseISRSolver2*);
double* extractVCSErrPointer(BaseISRSolver2*);
double* extractBCSPointer(BaseISRSolver2*);
...
class BaseISRSolver2 {
 public:
  BaseISRSolver2(std::size_t numberOfPoints,
                 double* energy,
                 double* visibleCS,
                 double* energyErr,
                 double* visibleCSErr,
   ...
   * @see InputOptions
   */
  BaseISRSolver2(const std::string& inputPath, const InputOptions& inputOpts);
  /**
   * Constructor
   * @param vcsGraph a visible cross section in a form of TGraphErrors
   * @param thresholdEnergy a threshold energy (GeV)
   */
  BaseISRSolver2(TGraphErrors* vcsGraph, double thresholdEnergy);
  /**
   * Constructor
   * @param vcsGraph a visible cross section in a form of TGraphErrors
   ...
   * @param thresholdEnergy a threshold energy (GeV)
   */
  BaseISRSolver2(TGraphErrors* vcsGraph,
                 const std::vector<TH1D*>& eff,
                 double thresholdEnergy);
  /**
   * Copy constructor
   */
  BaseISRSolver2(const BaseISRSolver2&);
  /**
  ...
  void push_back_eff(const std::string&);
 ...
 protected:
  ...
  const std::string& _getEffName() const;
  /**
 ...
 private:
   ...
   */
  std::function<double(double, std::size_t)> _efficiency;
  /**
   * detection efficiency
   */
  std::vector<TH1D*> _effHists;
  /**
   * numerical solution (Born cross section)
   */
  Eigen::VectorXd _bornCS;
  friend double* extractECMPointer(BaseISRSolver2*);
  friend double* extractECMErrPointer(BaseISRSolver2*);
  friend double* extractVCSPointer(BaseISRSolver2*);
  friend double* extractVCSErrPointer(BaseISRSolver2*);
  friend double* extractBCSPointer(BaseISRSolver2*);
};
